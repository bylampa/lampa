...
function checkSyncConflicts() {
    ...
    // Если включена синхронизация CUB, удаляем оригинальный параметр BYLAMPA и показываем заглушку
    if (cubSync) {
        $('div[data-name="acc_sync"]').remove();
        $('div[data-name="acc_sync_disabled"]').show();
    } else {
        // Если CUB выключен, проверяем существует ли оригинальный параметр
        if (!$('div[data-name="acc_sync"]').length) {
            // Если параметр был удален, пересоздаем настройки
            Lampa.Settings.update();
        }
        $('div[data-name="acc_sync_disabled"]').hide();
    }
    
    // Если включена синхронизация BYLAMPA, удаляем оригинальный параметр CUB и показываем заглушку
    if (byLampaSync) {
        $('div[data-name="account_use"]').remove();
        $('div[data-name="account_use_disabled"]').show();
    } else {
        // Если BYLAMPA выключен, проверяем существует ли оригинальный параметр
        if (!$('div[data-name="account_use"]').length) {
            // Если параметр был удален, пересоздаем настройки
            Lampa.Settings.update();
        }
        $('div[data-name="account_use_disabled"]').hide();
