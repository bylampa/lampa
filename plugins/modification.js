...
function checkSyncConflicts() {
    ...
    var focusedElement = $('.settings-param.focus, .settings-param[data-focused="true"]');
    ...
    setTimeout(function() {
        if (focusedName) {
            var elementToFocus = $('div[data-name="' + focusedName + '"]');
            if (elementToFocus.is(':visible') && elementToFocus.length) {
                Lampa.Controller.focus(elementToFocus);
            } else {
                // Если элемент скрыт, фокусируемся на первом видимом элементе
                var firstVisible = $('.settings-param:visible').first();
                if (firstVisible.length) {
                    Lampa.Controller.focus(firstVisible);
                }
            }
        } else {
            // Если фокуса не было, устанавливаем на первый видимый элемент
            var firstVisible = $('.settings-param:visible').first();
            if (firstVisible.length) {
                Lampa.Controller.focus(firstVisible);
            }
